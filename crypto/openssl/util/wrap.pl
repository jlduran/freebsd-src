...
BEGIN {
    ...
    require "${SRC_DIR}/crypto/openssl/util/perl/OpenSSL/Util.pm";
    OpenSSL::Util->import();
}
...
if ($ARGV[0] eq '-fips') {
    $std_openssl_conf = "${SRC_DIR}/crypto/openssl/test/fips-and-base.cnf";
    shift;
...
if ($ARGV[0] eq '-jitter') {
    $std_openssl_conf = "${SRC_DIR}/crypto/openssl/test/default-and-jitter.cnf";
    shift;
